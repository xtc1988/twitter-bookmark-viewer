# データベースマイグレーション手順（認証なし版）

## 概要

認証なしで動作するようにデータベーススキーマを変更する必要があります。
...
```sql
-- Remove authentication and user_id dependencies
...
-- Create new schema without user_id
...
1. Supabaseダッシュボードにアクセス: https://supabase.com/dashboard/project/azlcuxgourruwoxptwbr
...
⚠️ **重要**: このマイグレーションは既存のデータをすべて削除します。既にデータがある場合は、事前にバックアップを取ってください。

## 確認

マイグレーション後、以下のコマンドでテーブルが正しく作成されたか確認できます：
